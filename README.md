...
| 계층 | 뜻 | 개수 |
|---|---|---|
| **금기(hard stop)** | 해당 렌즈를 후보에서 완전히 제외 | 13 |
| **주의(caution)** | 감점 + 사유와 조치를 함께 표시 | 25 |
...
주소 뒤에 `?test=1` 을 붙이면 임상 로직 자체 검증 74건이 화면에 출력됩니다.
...
- **임상 로직 74건** — 시나리오(황반변성, 증식성 당뇨망막병증, 중증 녹내장, 안구표면질환, RK 후, 원추각막, 소대 불안정, 야간운전, 완벽주의 조합), 수치 경계값, 미입력 처리, 인계 코드 왕복(빈 입력·경계값·오타 교정·형식 불일치 거부), 역할별 스키마, 데이터 정합성
- **화면과 버튼 접근성** — 6개 화면 크기 × 3개 역할 × 4개 스크롤 위치에서 '결과 보기' 버튼이 실제로 보이고 눌리는지, 입력 필드가 눌리지 않고 끝까지 닿는지
- **역할 전환과 인계** — 환자 화면에서 넣은 값이 코드를 거쳐 상담 화면에서 **같은 1순위**를 만드는지, 잘못된 코드가 기존 입력을 지우지 않는지, `#h=` 링크로 열어도 채워지는지
